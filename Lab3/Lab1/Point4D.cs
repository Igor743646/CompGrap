...
namespace Geometry
{
    ...
    {
        public int[] polygonesIndexes;
        public Polygon4D[] polygones;
        ...
        public Vector4D Normal
        {
            get
            {
                if (_normal == null)
                {
                    if (polygonesIndexes == null)
                    {
                        throw new ArgumentNullException("No polygon indexes for calculating normal!");
                    }

                    Vector4D normal = new Vector4D(0, 0, 0, 0);

                    for (int i = 0; i < polygonesIndexes.Length; i++)
                    {
                        normal += polygones[polygonesIndexes[i]].Normal;
                    }

                    _normal = Vector4D.Normalize(normal);
                    return _normal;
                }
                else
                {
        ...
        public Point4D(double x, double y, double z, double w, Polygon4D[] pol) : this(x, y, z, w)
        {
            polygones = pol;
        }
